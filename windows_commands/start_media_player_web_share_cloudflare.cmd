...
set "TUNNEL_LOG=%TEMP%\media_player_cloudflare_tunnel.log"
...
if not exist "%CLOUDFLARED%" (
  ...
  echo %SCRIPT_DIR%start_media_player_web_share.cmd
  pause
  exit /b 1
)

echo Starting Local Media Player web-share mode on port 8767...
echo A second window will open with the Cloudflare public URL.
echo Close both windows when you are done sharing.
echo.

start "Local Media Player Web Share" cmd /k ""%PYTHON_EXE%" "%APP_DIR%\media_player.py" --media-dir "%MEDIA_DIR%" --host 0.0.0.0 --port 8767 --web-share"
...
start "Local Media Player Cloudflare Tunnel" powershell -NoExit -ExecutionPolicy Bypass -Command "& '%CLOUDFLARED%' tunnel --url http://127.0.0.1:8767 2>&1 | Tee-Object -FilePath '%TUNNEL_LOG%'"
