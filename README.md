...
organize a local music, video, and text archive through a web browser.
...
By default, editable local/LAN mode uses port `8766`. Read-only web-share mode
uses port `8767`.
...
Then open the editable local player:
...
and text archives in this kind of layout:
...
Missing optional folders, such as `Video` or `Interviews`, simply show empty
pages.
...
Lyrics live beside the song files. English lyrics are preferred when available.
...
Port `8767` is intended for read-only web sharing. Keep port `8766` for your
local editable player.
